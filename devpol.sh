...
# then show the cursor.  This fixes backspace / line-editing for every
# subsequent `read` prompt, regardless of what stty changes the TUI made.
allow_input() {
  if [[ -n "$ORIG_STTY" ]]; then
...
ensure_rootfs_writable() {
  ...
  # FIX: restore input so backspace works for this prompt
  allow_input
  read -r yn
  disallow_input
...
#
# Only called from do_apply(), NOT at startup.
# This keeps Chrome fully usable until the user explicitly applies policies.
#
setup_chrome_conf() {
  ...
  warn "(screen will flicker — come back with Ctrl+Alt+F2, then run Apply again)"
  sleep 2
  restart_ui
  sleep 4
}
...
# ── first-run setup ───────────────────────────────────────────────────────────
#
# FIX: setup_chrome_conf is NO LONGER called here.
# It is deferred to do_apply() so Chrome stays usable until the user
# actually needs to push a modified policy blob.
#
setup() {
  if [[ -f "$SETUP_STAMP" ]]; then
    ...
    [[ $all_present -eq 1 ]] && return   # nothing to do — skip straight to TUI
    warn "Some files missing — re-running setup..."
  fi

  echo ""
  info "=== First-run setup ==="
  # chrome_dev.conf is intentionally NOT touched here
  install_deps
  download_files
  touch "$SETUP_STAMP"
  echo ""
...
do_apply() {
  ...
  # FIX: chrome_dev.conf is patched HERE, only when the user explicitly
  # applies.  If the flag isn't present yet, Chrome restarts and the user
  # returns to VT2 and runs Apply a second time — at which point the grep
  # check at the top of setup_chrome_conf() short-circuits immediately.
  setup_chrome_conf
...
do_quick() {
  [[ ! -f "$JSON_FILE" ]] && die "no dump.json — run Dump first"
  # FIX: restore full terminal state so backspace works when typing a choice
  allow_input
  clear
...
run_tui() {
  ...
  while true; do
    ...
    if [[ "$key" == $'\x1b' ]]; then
      ...
      case $SEL in
        0) clear; do_dump
           # FIX: restore input before the "Press Enter" pause
           allow_input
        ...
        5) clear
           ...
           # allow_input was already here — backspace now works thanks to ORIG_STTY restore
